...
const MapPage = () => {
    ...
    const [listFilter, setListFilter] = useState('all');          // ← NOU
    ...
    const styleCountry = useCallback((feature) => {
        ...
        if (!status) {
            return { color: '#2f2e30', weight: 0.8, fillColor: '#1e293b', fillOpacity: 0.3 };
        }
        return { color: '#000000', weight: 1.2, fillColor: STATUS_COLORS[status], fillOpacity: 0.7 };
    }, [countryStatusMap]);
    ...
    const onEachCountry = useCallback((feature, layer) => {
        ...
        if (countryData) {
            ...
            layer.bindTooltip(`<strong>${name}</strong><br/><small>Click pentru a marca</small>`, { sticky: true });
        }
    ...
    const popupExistingStatus = useMemo(() => {
        ...
        const data = (popupInfo.code ? countryStatusMap[popupInfo.code] : null)
            || countryStatusMap[popupInfo.name?.trim().toLowerCase()];
    ...
    const countryList = useMemo(() => {
        ...
        for (const dest of destinations) {
            ...
            if (!key) continue;
            if (!map[key]) {
                map[key] = { country_name: dest.country_en || dest.country, status: dest.status };
            } else if (priority[dest.status] > priority[map[key].status]) {
                ...
                <div className="map-page-left">
                    <div className="map-fullsize">
                        <MapContainer
                            ...
                            scrollWheelZoom minZoom={3} maxZoom={10}
                        >
                            <TileLayer
                ...
                <div className="map-page-right">
                    ...
                    <div className="my-countries-header">
                        ...
                        <div className="list-filter-pills">
                            <button
                                className={`list-filter-pill ${listFilter === 'all' ? 'list-filter-pill--active-all' : ''}`}
                            ...
                            {['visited', 'planned', 'wishlist'].map((s) => (
                                <button
                                    key={s}
                                    className={`list-filter-pill ${listFilter === s ? `list-filter-pill--active-${s}` : ''}`}
                        ...
                        {loading ? (
                            ...
                            filteredCountryList.map((country) => (
                                <div key={country.country_name} className="dest-item">
                                    ...
                                    <button className="btn-remove" onClick={() => handleRemoveCountry(country.country_name)}>
                                        ✕
                                    </button>
                                </div>
                            ))
