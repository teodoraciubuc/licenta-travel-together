...
async function removeMapStatus(req, res) {
  ...
  if (!destinationId) return res.status(400).json({ message: 'destinationId required' });
  try {
    await pool.query('DELETE FROM "User_Map_Status" WHERE user_id = $1 AND destination_id = $2', [userId, destinationId]);
    return res.json({ ok: true });
  } catch (err) {
  ...
  try {
    const result = await pool.query(
      `SELECT id, name, country AS country_code, country_en, latitude, longitude
       ...
       ORDER BY
         CASE WHEN name ILIKE $2 THEN 1 WHEN country_en ILIKE $2 THEN 2 WHEN country ILIKE $2 THEN 3 ELSE 4 END,
...
async function setCountryStatus(req, res) {
  try {
    ...
    if (!countryName || !status) return res.status(400).json({ message: 'countryName and status required' });
    ...
    for (const dest of destinationsResult.rows) {
      ...
      if (existing.rows.length > 0) {
        ...
        await pool.query('INSERT INTO "User_Map_Status" (user_id, destination_id, status) VALUES ($1, $2, $3)', [userId, dest.id, status]);
      }
    }
    return res.json({ ok: true });
  } catch (err) {
...
async function removeCountryStatus(req, res) {
  try {
    ...
    if (!countryName) return res.status(400).json({ message: 'countryName required' });
    await pool.query(
      `DELETE FROM "User_Map_Status" WHERE user_id = $1
         AND destination_id IN (SELECT id FROM "Destinations" WHERE country ILIKE $2 OR country_en ILIKE $2)`,
      [userId, countryName]
    );
    return res.json({ ok: true });
  } catch (err) {
...
async function rateDestination(req, res) {
  try {
    ...
    if (!destinationId || rating == null) return res.status(400).json({ message: 'destinationId and rating required' });
    const r = Number(rating);
    if (!Number.isInteger(r) || r < 1 || r > 5) return res.status(400).json({ message: 'rating must be integer 1-5' });
    ...
    if (existing.rows.length === 0) {
      ...
      await pool.query('UPDATE "User_Map_Status" SET rating = $1 WHERE id = $2', [r, existing.rows[0].id]);
    }

    const boost = r >= 5 ? 2 : r === 4 ? 1 : r === 3 ? 0 : -1;
    if (boost !== 0) {
      const tagsResult = await pool.query('SELECT tag_id FROM "Destination_Tags" WHERE destination_id = $1', [destinationId]);
      for (const row of tagsResult.rows) {
        ...
        if (prefExists.rows.length > 0) {
          ...
          await pool.query('UPDATE "User_Preferences" SET score = $1 WHERE id = $2', [newScore, prefExists.rows[0].id]);
        } else {
          await pool.query('INSERT INTO "User_Preferences" (user_id, tag_id, score) VALUES ($1, $2, $3)', [userId, row.tag_id, Math.min(5, Math.max(1, 3 + boost))]);
        }
      }
    }
 ...
 * Lets users add a city not in the DB.
 * body: { name, country, countryEn, latitude?, longitude?, status }
 *
 * - Inserts into Destinations (user_submitted = true)
 * - Adds to User_Map_Status with given status
 * - If the city already exists (same name + country), reuses it
 */
async function suggestDestination(req, res) {
  try {
    const userId = req.user?.id || req.user?.userId;
    const { name, country, countryEn, latitude, longitude, status = 'visited' } = req.body;
    ...
    const cleanEn      = (countryEn || country).trim();
    ...
    const existing = await pool.query(
      `SELECT id FROM "Destinations"
       WHERE name ILIKE $1 AND (country ILIKE $2 OR country_en ILIKE $2)`,
      [cleanName, cleanCountry]
    );
    ...
    if (existing.rows.length > 0) {
      ...
      destinationId = existing.rows[0].id;
    } else {
      // Insert new destination marked as user_submitted
      const inserted = await pool.query(
        `INSERT INTO "Destinations"
           (name, country, country_en, latitude, longitude, user_submitted, submitted_by)
         VALUES ($1, $2, $3, $4, $5, TRUE, $6)
         RETURNING id`,
        [cleanName, cleanCountry, cleanEn, lat, lng, userId]
      );
      destinationId = inserted.rows[0].id;
    }

    // Add to user's map
    const mapEntry = await pool.query(
      'SELECT id FROM "User_Map_Status" WHERE user_id = $1 AND destination_id = $2',
      [userId, destinationId]
    );
    if (mapEntry.rows.length > 0) {
      await pool.query('UPDATE "User_Map_Status" SET status = $1 WHERE id = $2', [status, mapEntry.rows[0].id]);
    } else {
      await pool.query(
        'INSERT INTO "User_Map_Status" (user_id, destination_id, status) VALUES ($1, $2, $3)',
        [userId, destinationId, status]
    ...
    return res.status(201).json({ ok: true, destinationId, name: cleanName, country: cleanEn });
  } catch (err) {
    console.error('suggestDestination error:', err);
    return res.status(500).json({ message: 'Database error' });
  }
}

module.exports = {
  getMyMap, setMapStatus, removeMapStatus, searchDestinations,
  setCountryStatus, removeCountryStatus, rateDestination, suggestDestination,
};